...
python3 manage.py shell -c "from core.models import User;User.objects.filter(username='${SUPERUSER_NAME}').exists() or User.objects.create_superuser('${SUPERUSER_NAME}', '${SUPERUSER_MAIL}', '${SUPERUSER_PASSWORD}')"
