...
test:
	$s docker exec ${PROJECT_NAME}_backend pytest -n ${PYTEST_WORKERS} --maxfail=5 -vv -s --log-cli-level=ERROR --disable-pytest-warnings --create-db --ds backend.settings.test_settings

fasttest:
	$s docker exec ${PROJECT_NAME}_backend pytest -n ${PYTEST_WORKERS} --maxfail=5 -vv -s --log-cli-level=ERROR --disable-pytest-warnings --reuse-db --ds backend.settings.test_settings
...
create-nginx-conf:
	...
	$s sed -e "s/NGINX_EXTERNAL_PORT/${NGINX_EXTERNAL_PORT}/g" docker/nginx/nginx.conf-tmp > docker/nginx/nginx.conf-final
